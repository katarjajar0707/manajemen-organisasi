...
import { Megaphone, Plus, Calendar, Bell, Search, MoreVertical, Edit, Trash2, Loader2, AlertCircle, Users, Building2 } from 'lucide-react';
import { PengumumanItem, createPengumuman, updatePengumuman, deletePengumuman } from '@/actions/pengumuman';
...
export function PengumumanManager({ initialAnnouncements, bagianList, currentUserId, userRole = 'anggota', currentUserBagianId }: PengumumanManagerProps) {
  ...
  const [target, setTarget] = useState<'semua' | 'bagian_tertentu'>(userRole === 'admin' || userRole === 'ketua' ? 'semua' : 'bagian_tertentu');
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-card border rounded-xl p-3 shadow-xs">
        <div className="flex items-center gap-1.5 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0">
          <Button size="sm" variant={filterTarget === 'all' ? 'default' : 'ghost'} className="text-xs h-8 px-3 rounded-lg whitespace-nowrap shrink-0" onClick={() => setFilterTarget('all')}>
            Semua ({announcements.length})
          </Button>
          <Button size="sm" variant={filterTarget === 'semua' ? 'default' : 'ghost'} className="text-xs h-8 px-3 rounded-lg gap-1.5 whitespace-nowrap shrink-0" onClick={() => setFilterTarget('semua')}>
            <Users className="h-3.5 w-3.5" />
            <span>Semua Anggota ({announcements.filter((a) => a.target === 'semua').length})</span>
          </Button>
          <Button size="sm" variant={filterTarget === 'bagian_tertentu' ? 'default' : 'ghost'} className="text-xs h-8 px-3 rounded-lg gap-1.5 whitespace-nowrap shrink-0" onClick={() => setFilterTarget('bagian_tertentu')}>
        ...
        <div className="relative w-full sm:w-64 flex-1">
          ...
          <Input placeholder="Cari judul atau isi pengumuman..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-8 h-8 text-xs bg-muted/30 w-full" />
        </div>
      </div>
        ...
        {filteredAnnouncements.length === 0 ? (
          ...
          filteredAnnouncements.map((item) => (
            <Card key={item.id} className="hover:shadow-sm transition-shadow">
              ...
              <CardContent>
                <p className="text-sm text-muted-foreground leading-relaxed whitespace-pre-line">{item.isi}</p>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-lg w-[95vw] sm:w-full">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs">Pesan ini bersifat broadcast satu arah untuk pemberitahuan resmi.</DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              ...
              <Select value={target} onValueChange={(val: 'semua' | 'bagian_tertentu') => setTarget(val)} disabled={!canBroadcastAll}>
              ...
              {!canBroadcastAll && <p className="text-[11px] text-muted-foreground">*Sebagai anggota bagian, pengumuman otomatis ditujukan ke bagian Anda.</p>}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <Input value={judul} onChange={(e) => setJudul(e.target.value)} placeholder="Contoh: Pemberitahuan Rapat Kerja Bakti Minggu Ini" className="text-xs" />
            </div>
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button size="sm" onClick={handleSave} loading={isPending} disabled={!judul.trim() || !isi.trim()}>
      ...
      <Dialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs">Apakah Anda yakin ingin menghapus pengumuman ini?</DialogDescription>
          </DialogHeader>
          <DialogFooter className="gap-2 sm:gap-0">
