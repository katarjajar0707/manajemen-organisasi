...
export function AppMobileNav({
  ...
  return (
    <div className="fixed inset-0 z-50 lg:hidden flex">
      {/* Backdrop */}
      <div className="fixed inset-0 bg-black/60 backdrop-blur-xs transition-opacity duration-200 animate-in fade-in" onClick={() => setMobileOpen(false)} aria-hidden="true" />

      {/* Drawer Panel */}
      <aside className={cn('relative z-10 w-[85vw] max-w-72 bg-background border-r border-border h-full flex flex-col shadow-2xl', 'animate-in slide-in-from-left duration-250 ease-out')}>
        {/* Drawer Header */}
        <div className="flex h-16 items-center justify-between px-4 border-b border-border/70 shrink-0">
          ...
          <Button variant="ghost" size="icon" onClick={() => setMobileOpen(false)} className="h-8 w-8 text-muted-foreground hover:text-foreground rounded-lg">
            <X className="h-5 w-5" />
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-5 pb-8">
          ...
          {ADMIN_NAV_ITEMS.some((item) => isAuthorized(item.roles)) && (
            <div>
              <p className="px-3 mb-1.5 text-[11px] font-bold uppercase tracking-wider text-muted-foreground/70">Administrasi Khusus</p>
              <nav className="space-y-1">
