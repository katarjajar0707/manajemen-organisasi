...
export function ColorThemeSwitcher({ isCollapsed = false, showDivider = true, compact = false, label = 'Color theme' }: { isCollapsed?: boolean; showDivider?: boolean; compact?: boolean; label?: string }) {
  ...
  return (
    <div className={cn(compact ? 'bg-transparent' : 'mt-auto bg-sidebar/80 backdrop-blur-sm', !compact && showDivider && 'border-t border-border/70', compact ? '' : isCollapsed ? 'px-2 pb-2 pt-2' : 'px-3 py-3')}>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button
            ...
            className={cn(
              ...
              compact && 'min-h-16 justify-start px-3 py-2.5 sm:min-h-20 sm:px-4 sm:py-3',
            )}
          >
            <span className="flex min-w-0 items-center gap-2.5">
              ...
              {!isCollapsed && (
                <span className="min-w-0 leading-tight">
                  <span className="block text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground/70">{label}</span>
