...
function formatRelativeTime(dateIso: string): string {
  try {
    ...
    return new Date(dateIso).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' });
  } catch {
    return 'Baru saja';
  }
}
...
export function AnggotaDashboardShell({ profile, children }: { profile: Profile; children: React.ReactNode }) {
  return (
        ...
        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="grid grid-cols-3 gap-2 sm:flex sm:flex-wrap sm:items-center">
            ...
            <Link href="/kegiatan" className="min-w-0">
              <Button size="sm" className="w-full gap-1.5 bg-primary px-2 text-xs text-primary-foreground shadow-[0_0_16px_var(--primary-glow)] hover:bg-primary/90 sm:w-auto sm:gap-2 sm:px-3">
...
export function AnggotaDashboardProfile({ profile }: { profile: Profile }) {
  return (
      ...
      <CardContent className="space-y-5 pt-5">
        ...
        <div className="space-y-0 rounded-xl border border-border/60 bg-muted/20 px-3">
          ...
          <div className="flex items-center justify-between gap-3 py-2.5">
            ...
            <span className="text-xs">{new Date(profile.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
          </div>
        </div>

        {profile.bio && <p className="border-l-2 border-primary/30 py-1 pl-3 text-xs italic text-muted-foreground">&ldquo;{profile.bio}&rdquo;</p>}
    ...
    <div>
      ...
      <div className="grid gap-3 sm:grid-cols-3 sm:gap-4">
        <Link href="/keuangan" className="block">
          <Card className="h-full border-border/70 transition-all duration-200 hover:-translate-y-0.5 hover:border-emerald-500/40 hover:shadow-md">
            <CardContent className="flex items-center gap-4 p-4 sm:p-5">
              <div className="shrink-0 rounded-xl bg-emerald-500/10 p-3 text-emerald-600"><Wallet className="h-5 w-5" /></div>
              <div className="min-w-0">
                <div className="text-xl font-bold tracking-tight truncate">{formatRupiah(summaryData.keuangan.saldoAkhir)}</div>
                <p className="flex items-center gap-1 text-xs text-muted-foreground"><TrendingUp className="h-3 w-3 text-emerald-600" /> Saldo kas organisasi</p>
              </div>
            </CardContent>
          </Card>
        </Link>
        <Link href="/anggota" className="block">
          <Card className="h-full border-border/70 transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-md">
            <CardContent className="flex items-center gap-4 p-4 sm:p-5">
              <div className="shrink-0 rounded-xl bg-primary/10 p-3 text-primary"><Users className="h-5 w-5" /></div>
              <div>
        ...
        <Link href="/kegiatan" className="block">
          <Card className="h-full border-border/70 transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-md">
            <CardContent className="flex items-center gap-4 p-4 sm:p-5">
              <div className="shrink-0 rounded-xl bg-primary/10 p-3 text-primary"><Calendar className="h-5 w-5" /></div>
              <div>
      ...
      <AnggotaFinanceChart data={summaryData.keuangan} />
    </div>
  );
}
  ...
  return (
    <Card className="border-border/70 shadow-sm lg:col-span-3">
      <CardHeader className="flex flex-row items-start justify-between gap-3 border-b border-border/60 pb-4">
        <div>
          <CardTitle className="flex items-center gap-2 text-base"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-primary/10 text-primary"><Sparkles className="h-4 w-4" /></span>Jadwal Kegiatan</CardTitle>
          <CardDescription className="pl-10 text-xs">Kegiatan terbaru yang direncanakan organisasi</CardDescription>
        </div>
        <Link href="/kegiatan"><Button variant="ghost" size="sm" className="shrink-0 gap-1 text-xs text-primary hover:text-primary">Semua <ArrowRight className="h-3 w-3" /></Button></Link>
      </CardHeader>
      <CardContent className="space-y-2.5 pt-5">
        {kegiatanMendatang.length === 0 ? (
          <div className="rounded-xl border border-dashed border-border/80 bg-muted/20 py-10 text-center text-muted-foreground"><Calendar className="mx-auto mb-2 h-8 w-8 opacity-40" /><p className="text-sm font-medium text-foreground">Belum ada jadwal mendatang</p><p className="mt-1 text-xs">Agenda baru akan tampil di sini setelah dipublikasikan.</p></div>
        ) : kegiatanMendatang.map((kegiatan) => (
          <Link key={kegiatan.id} href="/kegiatan" className="block group">
            <div className="flex items-start gap-3 rounded-xl border border-border/60 bg-card/60 p-3 transition-all hover:border-primary/30 hover:bg-primary/3">
              <div className="mt-0.5 shrink-0 rounded-lg bg-primary/10 p-2 text-primary"><Calendar className="h-3.5 w-3.5" /></div>
              <div className="min-w-0 flex-1"><p className="font-medium text-sm line-clamp-1 group-hover:text-primary transition-colors">{kegiatan.judul}</p><p className="mt-0.5 text-[11px] text-muted-foreground line-clamp-1">{kegiatan.tanggal}</p>{kegiatan.lokasi && <div className="mt-0.5 flex items-center gap-1 text-[11px] text-muted-foreground"><MapPin className="h-2.5 w-2.5 shrink-0" /><span className="truncate">{kegiatan.lokasi}</span></div>}</div>
              <Badge variant="outline" className="shrink-0 border-primary/25 bg-primary/5 text-[10px] text-primary">{kegiatan.bagian}</Badge>
            </div>
          </Link>
        ))}
      </CardContent>
...
export async function AnggotaDashboardDiscussions() {
  ...
  return (
    <Card className="border-border/70 shadow-sm">
      <CardHeader className="flex flex-row items-start justify-between gap-3 border-b border-border/60 pb-4">
        <div>
          <CardTitle className="flex items-center gap-2 text-base"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-primary/10 text-primary"><MessagesSquare className="h-4 w-4" /></span>Diskusi & Catatan Umum Terbaru</CardTitle>
          <CardDescription className="pl-10 text-xs">Ikut berpartisipasi dalam diskusi organisasi</CardDescription>
        </div>
        <Link href="/diskusi"><Button variant="ghost" size="sm" className="shrink-0 gap-1 text-xs text-primary hover:text-primary">Lihat Semua <ArrowRight className="h-3 w-3" /></Button></Link>
      </CardHeader>
      <CardContent className="space-y-3 pt-5">
        {latestDiskusis.length === 0 ? (
          <div className="rounded-xl border border-dashed border-border/80 bg-muted/20 py-10 text-center text-muted-foreground"><MessagesSquare className="mx-auto mb-2 h-8 w-8 opacity-40" /><p className="text-sm font-medium text-foreground">Belum ada diskusi terbaru</p><Link href="/diskusi" className="mt-4 inline-block"><Button size="sm" variant="outline" className="gap-2 text-xs"><MessageCircle className="h-3.5 w-3.5" />Mulai Diskusi</Button></Link></div>
        ) : <div className="grid gap-2.5 sm:grid-cols-2">{latestDiskusis.map((diskusi) => (
          <Link key={diskusi.id} href={`/diskusi/${diskusi.id}`} className="block group">
            <div className="space-y-2 rounded-xl border border-border/60 bg-card/60 p-3.5 transition-all hover:border-primary/30 hover:bg-primary/3">
              <div className="flex items-center justify-between gap-2"><h4 className="font-medium text-sm line-clamp-1 group-hover:text-primary transition-colors">{diskusi.judul}</h4>{diskusi.mentions?.length > 0 && <Badge variant="outline" className="shrink-0 text-[10px] text-primary">@{diskusi.mentions[0].bagianNama}</Badge>}</div>
              <div className="flex items-center justify-between text-xs text-muted-foreground"><span className="flex items-center gap-1"><Clock className="h-2.5 w-2.5" />{formatRelativeTime(diskusi.createdAt)}</span><span className="flex items-center gap-1 text-[11px]"><MessageCircle className="h-3 w-3" />{diskusi.balasanCount}</span></div>
            </div>
          </Link>
        ))}</div>}
      </CardContent>
