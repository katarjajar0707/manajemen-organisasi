...
export function DiskusiManager({
  ...
  const handleTogglePin = (t: DiskusiItem) => {
    startTransition(async () => {
      ...
      if (res.success) {
        setThreads((prev) =>
          prev.map((item) => (item.id === t.id ? { ...item, isPinned: newPinned } : item))
        );
      } else {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Forum komunikasi terbuka antar bagian, catatan bersama lintas divisi, dan mention @departemen.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
          <Button
            size="sm"
            variant="outline"
            className="gap-2 shadow-xs flex-1 sm:flex-initial text-xs h-8"
          ...
          <Button
            size="sm"
            className="gap-2 shadow-xs bg-primary hover:bg-primary/90 flex-1 sm:flex-initial text-xs h-8"
              ...
              >
                <CardHeader className="pb-3">
                  ...
                  <CardDescription className="text-xs">
                    Ditulis oleh <span className="font-semibold text-foreground">{t.authorName}</span> (
                    {t.authorRole} • {t.bagianPembuatNama || "Umum"})
                  </CardDescription>
                </CardHeader>
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap pt-1">
                      ...
                      {t.mentions.map((m) => (
                        <Badge
                          key={m.id}
                          variant="secondary"
                          className="text-[11px] font-medium text-primary bg-primary/10 hover:bg-primary/20"
                        >
                          @{m.bagianNama}
                        </Badge>
                      ))}
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      className={`h-8 px-2.5 text-xs gap-1.5 ${
                        isLiked ? "text-rose-500 hover:text-rose-600 font-medium" : "text-muted-foreground"
                      }`}
      ...
      <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
        <DialogContent className="max-w-xl w-[95vw] sm:w-full max-h-[90vh] overflow-y-auto">
            ...
            <DialogTitle className="flex items-center gap-2 text-lg">
              ...
              <span>
                {editingThread
                  ...
                  : "Tulis Catatan Bersama"}
              </span>
            </DialogTitle>
            <DialogDescription className="text-xs">
              Posting pesan ini untuk mengundang masukan dari pengurus atau menyimpan SOP/catatan penting lintas bagian.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button size="sm" onClick={handleSave} disabled={isPending || !judul.trim()}>
